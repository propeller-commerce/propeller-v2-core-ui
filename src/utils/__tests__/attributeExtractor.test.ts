...
import type { AttributeResult, Product } from '@propeller-commerce/propeller-sdk-v2';
...
describe('extractAttributeValues — current type-based format', () => {
  ...
  it('extracts ENUM values from enumValues (the schema field)', () => {
    // AttributeEnumValue exposes its values on `enumValues`, never `value` —
    // reading `value` returned [] and left ENUM-spanned cluster configurators
    // with empty dropdowns. This is the primary path.
    expect(
      extractAttributeValues(attr({}, { type: AttributeType.ENUM, enumValues: ['WIFI', 'ETHERNET', ''] })),
    ).toEqual(['WIFI', 'ETHERNET']);
  });

  it('falls back to a legacy ENUM value when enumValues is absent', () => {
