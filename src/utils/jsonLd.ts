...
export function buildProductJsonLd(
  ...
  const categoryName = getLanguageString(product.category?.names, ctx.language, '').trim();
...
export function buildClusterJsonLd(
  ...
  const absoluteImage = imageUrl ? toAbsolute(ctx.siteUrl, imageUrl) : undefined;
  const categoryName = getLanguageString(cluster.category?.names, ctx.language, '').trim();
...
export function buildItemListJsonLd(
  ...
  products.forEach((item, index) => {
    ...
    const isCluster = item.type === ProductClass.CLUSTER;
