...
export function extractAttributeValues(attr: AttributeResult): string[] {
  ...
  // ── Current SDK format (type-based) ─────────────────────────────────────
    ...
    // The schema exposes enum values as `enumValues: [String]` and never
    // populates `value` on AttributeEnumValue (see the SDK
    // `AttributeEnumValueFields` fragment). Reading `value` here returned `[]`
    // for every ENUM attribute, so ENUM-spanned cluster configurators rendered
    // empty dropdowns.
    if (Array.isArray(v.enumValues)) {
      return (v.enumValues as string[]).filter(Boolean);
    }
    if (v.value) values.push(v.value);
  }
