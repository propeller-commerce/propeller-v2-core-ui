...
visitor for the first frames — without the flag, a signed-in shopper sees the
log-in call to action and blanked prices until `getViewer()` resolves. Note the
host's own `isLoading` is already `false` by then, so it cannot serve this
purpose; the signal has to be "a session exists".

It is optional, and omitting it keeps the previous behaviour — server renders,
which have no such window, pass two arguments as before.

```ts
// Host bridge: feed it alongside `user`.
<PropellerProvider user={state.user} isAuthenticated={state.isAuthenticated} … />
```
